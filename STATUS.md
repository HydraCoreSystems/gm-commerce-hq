...
- **Product Reset Revision 3 (corrected) published, awaiting Copilot's focused re-review:** responds to all 15 finite corrections and 7 canonical decisions from the Revision 2 review (`docs/autonomous-commerce-intelligence-audit.md`) — canonical entity hierarchy (17 types), marketplace-neutral `CommercePackage`, `SourceConnector` contract, Intelligence Repository service contract, catalog-scale requirements, correction-scope inference, and Phase B0 review-shell sequencing are each now a concrete specification (interfaces/schemas/state machines) in `PRODUCT_RESET_2026-08-03.md`, not a restated intention. Also resolves a real documentation-state discrepancy Copilot found: Revision 2's claimed "Phase A" work (Commerce Readiness Gate, `commerce_details`, `content_provenance`) exists only in a local, uncommitted `gm-commerce` working tree — including a live-database schema change with no corresponding git migration — now documented component-by-component with an honest retain/migrate/discard disposition per component, not committed as part of this update. **Same-day correction:** the first push (`b2feb517`) itself mischaracterized `HY-LOB01-C04`'s test-session `commerce_details` as real production data; Phil caught this directly and it's corrected in place, plus a new enforced `RecordContext`/environment-isolation model (§25) and a database-change-control policy (§1.1) prevent this class of mistake structurally going forward — see the revised commit's changelog. `HY-LOB01-C04`'s test records are untouched pending Phil's explicit direction on quarantine/disposition. Awaiting Copilot's re-review of the corrected commit: `APPROVE FOR IMPLEMENTATION` / `APPROVE AFTER DOCUMENTED CORRECTIONS` / `REQUIRES REVISION 4`. Etsy and production implementation remain paused.
