...
**Honest limitations carried forward, disclosed rather than glossed over:** `canonical_claims.sources`/`.evidence_anchor_ids` carry no database foreign key (jsonb array / text[] elements — Postgres has no native per-element FK mechanism without a join table + trigger per element; the application layer validates every referenced id exists in the caller's own environment before insert, same tradeoff already accepted for slice 1's `applicability` jsonb column). Independent-source-family detection is an explicit, ingestion-assigned label, not automated content-similarity comparison (deferred to a real research-ingestion connector). Full detail in `handoffs/2026-08-03-phase-b-slice-2-evidence-claims.md`.

**Correction pass round 1 (2026-08-03, same PR): Codex's first review returned `NOT APPROVED TO MERGE` with twelve blocking findings** — caller-asserted verifier identity, unproven owner-override genuineness, fabricable source-family/evidence-anchor claims, a wrong botanical/care authority test, freshness failing open, an unscoped contradiction-history query, contradiction/supersession operations that didn't check relatedness, no durable verification audit, non-atomic multi-write operations, and an unchecked pointer-update result. All twelve addressed on the same branch (commits `7bb7166`/`0e86374`), each with its own fix and new tests — full detail in the PR's follow-up comment and the handoff doc. Notably: `verifyClaim`/`resolveContradiction` now take a plain actor id resolved through a new trusted actor-role registry (`lib/canonical/claims/actor-registry.ts`, config-based, fails closed) rather than accepting a caller-supplied `ActorRef` — the RBAC limitation noted in the paragraph above is now closed. A new `recordOwnerDecision` operation is the one privileged, audited path that can create a genuinely-approved `OwnerDecision` (generic entity creation still structurally cannot, by slice 1's own design). Four new atomic Postgres functions replace what were previously multiple independent statements for `ingestEvidence`/`recordContradiction`/`resolveContradiction`/`verifyClaim`'s writes, live-verified with forced-failure rollback tests against real Postgres and added as permanent CI steps. New test/verification total: **443 passed / 0 failed** (up from 378). Not merged — awaiting Codex's re-review.

**Correction pass round 2 (2026-08-03, same PR): Codex's re-review found round 1 fixed the mechanical bugs but left load-bearing trust defects — `STILL NOT APPROVED TO MERGE`, eight architecture-level findings.** Codex's own closing instruction shaped the fix: "when a prerequisite such as authenticated owner identity or trusted SourceConnector attribution is not implemented in Slice 2, the correct Slice 2 behavior is to fail closed and defer the privileged transition" — not invent something that merely resembles the missing mechanism. Concretely (commit `982973f`):
  - **#1 (the central fix):** round 1's `actor-registry.ts`/`ActorRef` — a plain `{id, role}` pair a caller could construct after a role lookup — is deleted entirely, replaced by a `CallerPrincipal` (new `caller-principal.ts`) **bound at repository construction**, never accepted as a per-call parameter (mirroring how `Environment` is already bound). Since no real authenticated-owner session layer exists anywhere in this codebase, `resolveTrustedCallerPrincipal()` can only ever return `{ kind: "service" }` today — so `recordOwnerDecision` and `resolveContradiction` now throw `UNAUTHORIZED` **unconditionally**, and `verifyClaim` only ever reaches its `auto_eligible` branch. This is deliberate, not a regression: those operations are correctly unreachable until a later phase adds real owner-session auth.
  - **#2/#6:** authoritative-source-of-record classification (`isAuthoritativeSystemOfRecord`) is now gated behind an explicit `SOURCE_CONNECTOR_ATTESTATION_AVAILABLE` toggle (hardcoded `false` — no `SourceConnector`, §8, exists yet) plus a predicate-specificity allowlist (only `commerce.weight`, a bare physical fact — never price/compliance/safety/marketplace-policy, which are business decisions regardless of source).
  - **#3:** a new `assessSourceFamilyEquivalence` operation + `gmcom_assess_source_family` RPC is now the *only* way a source-family relationship can ever be recorded (`ingestEvidence`'s signature no longer even accepts one); an unassessed `EvidenceSource` resolves to a sentinel that contributes **zero** toward the independent-family corroboration count, never "independent by default."
  - **#4:** `canonical_owner_decisions` gained `predicate`/`applicability_scope`; `proposeClaim` now requires the referenced `OwnerDecision`'s predicate and scope to match the claim exactly (not just its subject), and any claim citing an `owner_exception` source with no `ownerOverrideId` is rejected outright.
  - **#5:** `community_report` claims now actually require a higher-authority evidence type cited alongside them before reaching `requires_authorized_verifier` — an uncorroborated one stays `insufficient` indefinitely.
  - **#7:** `gmcom_verify_claim`/`gmcom_resolve_contradiction` independently re-lock and recheck status/open-contradiction/freshness/evidence-snapshot-integrity/membership/compatibility inside their own transaction, so a direct RPC call bypassing the TypeScript class entirely still can't force an unjustified state change — exercised in tests via direct `supabase.rpc()` calls (since the TS entry points are themselves unreachable per #1) and live-verified against real Postgres, with the CI `schema-from-empty` job extended to match.
  - **#8:** corrected `types.ts`'s now-stale `ownerOverride` doc comment.

New test/verification total: **445 passed / 0 failed** (up from 443; net +67 counting rewrites of round-1 tests whose success paths are now intentionally unreachable). `npm run typecheck`/`npm run build` both clean. Not merged — awaiting Codex's next re-review; PR comment posted with the full breakdown.
