...
> **Slice status:** **I1, I2, and I3 are delivered and merged** — PR #54, merge SHA `2c26b61`; **PR #55, merge SHA `bef5a5d`**; **PR #56, merge SHA `285a2a0`** on `gm-commerce/main`. The identity backbone is complete. **The next Phase I slice has not begun** and remains pending design and owner authorization; the likely next boundary is legacy approved-photo context into the canonical PhotoAsset model (to be verified against the merged schema and prior decisions, not assumed). Later slices are not begun.
...
The **architecture itself is approved by the owner as the next phase** (owner decision 11). Slices are independently testable and mergeable. **I1, I2, and I3 are delivered and merged (PR #54, `2c26b61`; PR #55, `bef5a5d`; PR #56, `285a2a0`); no later Phase I slice has been implemented.**
...
| Later | PhotoAsset after approved photo sets (proposed I4: approved photo set → canonical PhotoAsset, attached assets only) | Design recommendation only — not authorized (7 owner-gated decisions) | After I3 |
...
- **Proposed I4 (design recommendation only — NOT authorized):** approved photo-set → canonical PhotoAsset (attached assets only, no Claims/Evidence/CommercePackage). A read-only design inventory has been produced and grounded in the merged code at `gm-commerce/main` `285a2a0` (see STATUS.md / handoff addendum §10). **This slice is a design recommendation only and is not authorized; it requires owner review of the seven blocking decisions below.** The inventory established: the truthfully-approved legacy event is `photo_sets.status='approved'` (guarded `needs_review→approved` with `approved_at`/`approved_by`, the durable terminal state that gates `publishToShopify`); `products.photos_confirmed_at` is a distinct, earlier intake attestation, not photo approval; photos are **attached/commerce assets** (DECISIONS.md "Photos are a first-class commerce asset"; PRODUCT_RESET §5/§10/§17) — not knowledge Evidence and not Claims; `canonical_photo_assets` has **no `status`/`approved_at`** and cannot truthfully carry approval (the generic `createEntity` cannot set `owner_approval_state='genuine'`), so canonical PhotoAssets remain `'pending'` and the legacy approved photo set remains the legacy source of truth.
- **Owner decisions blocking I4 (unresolved, owner-gated):**
  1. Photos are attached commerce assets, not Claims or Evidence.
  2. Canonical identity unit: the original legacy `photo_assets` row vs its `photo_derivatives` representations. **Smallest-safe provisional:** one canonical PhotoAsset per stable legacy **original** photo asset; preserve derivative metadata/references without separate canonical identities unless the canonical contract explicitly treats each derivative as an independent PhotoAsset. If the schema cannot retain derivative references without an additive field/table, that is part of the I4 design — not permission to invent or discard them.
  3. Representation of derivative paths/metadata (originals vs derivatives; whether an additive field/table is justified).
  4. Canonical PhotoAssets remain `owner_approval_state='pending'`; the legacy approved photo set remains the legacy source of truth.
  5. Exact stable `storage_ref` format (content-hash-based vs normalized path; no binary copy/move).
  6. **Durable per-photo bridge mapping is required** — the run/outcome ledger is execution history, not a permanent identity mapping; I4 needs a durable legacy-photo→canonical-PhotoAsset mapping for idempotent replay, concurrency convergence, drift detection, mismatch reporting, and later CommercePackage media references. Smallest-safe: additively widen `canonical_legacy_entity_bridge.legacy_table` to allow the verified legacy per-photo table, one permanent bridge row per canonical PhotoAsset, and keep the run/outcome ledger separate for operational history. (No prior migration modified; the widening is an additive I4 migration.)
  7. Ongoing approval-event integration (enqueue at `approvePhotoSet`) vs photo backfill ordering — **recommend ongoing integration first**, consistent with Phase I's established production-before-backfill discipline (owner decision 6), but not authorized yet.
- **Block later slices (not I4):** potential future **image-region evidence** use — an EvidenceAnchor whose `ImageRegion` locator references a canonical PhotoAsset (separate from, and not part of, I4); CommercePackage creation timing (owner decision 10) and content-availability; **SourceCategory/evidence design for AI-content Claims (owner decision 8 — the only hard owner-gated blocker)**; any new SourceCategory (deliberate, never invented).
